...
public class StreamsExample {
	...
	public static void main(String[] args) {
		...
		people.stream().filter(p -> p.getLastName().startsWith("H")).forEach(p -> System.out.println(p.getFirstName()));

		long count = people.stream().filter(p -> p.getLastName().startsWith("C")).count(); // count
																							// =
																							// terminal
																							// operation
